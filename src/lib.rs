...
pub fn create_coinjoin_transaction(/* parameters */) -> Result<(transaction::CoinjoinTransaction, ZKProof<Bn256>), String> {
    ...
    let proof: Result<ZKProof<Bn256>, Box<dyn std::error::Error>> = generate_proof(&transaction, &nonce);
...
pub fn verify_coinjoin_transaction(transaction: &transaction::CoinjoinTransaction, proof: &ZKProof<Bn256>) -> bool {
