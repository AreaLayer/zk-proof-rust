...
pub fn create_coinjoin_transaction(/* parameters */) -> Result<(transaction::CoinjoinTransaction, ZKProof<pairing::bn256::Bn256>), String> {
    ...
    let proof: Result<ZKProof<pairing::bn256::Bn256>, Box<dyn std::error::Error>> = generate_proof(&transaction, &nonce);
